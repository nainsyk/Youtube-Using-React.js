  ...
  return (
    <div className="grid grid-flow-col shadow-lg">
       <div className="flex col-span-1 ">
        ...
        <div>
       <div className="px-10 mt-2 col-span-10 mr-24">
           <input 
             className="w-[75%] px-11 border-[0.5px] border-r-0 border-gray-30 outline-none py-[0.5rem] rounded-l-full"
             type="text"
             placeholder="Search"
             value={searchQuery}
             ...
             />
           <button className="border border-gray-300 px-7 pt-[0.63rem] pb-[0.88rem] rounded-r-full bg-gray-100"><TfiSearch />
</button>
          <button className="h-12 ml-5 px-4 border border-gray-100 bg-gray-200 rounded-full"><FaMicrophone />
</button>
       </div>
       {showSuggestions && (
       <div className="fixed ml-11 bg-white w-[27.8rem] shadow-lg rounded-2xl">
        <ul>
          {suggestions.map((s) => (
            <li key={s} className="border-gray-100 flex py-2 px-5 gap-2 shadow-sm hover:bg-gray-100"><TfiSearch className="mt-1" />{s}</li>
          ))}
        </ul>
        </div>
        )}
